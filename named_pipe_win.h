...
struct NamedPipeWin {
 public:
  static constexpr unsigned kDefaultBufferSize = 1024; // 1k
  // We can take more input arguments when we need them
  [[nodiscard]] static NamedPipeWin CreateServerPipe(const std::string& name, unsigned buf_size = kDefaultBufferSize);
  [[nodiscard]] static NamedPipeWin CreateClientPipe(const std::string& name);
  NamedPipeWin() = default;
  ~NamedPipeWin();
  NamedPipeWin(NamedPipeWin&&) = default; // disable copy
  NamedPipeWin& operator=(NamedPipeWin&&) = default; //disable assign
